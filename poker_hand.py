"""This program classifies a poker hand from a list of cards. It can
print the poker hand and compare it to a different one to identify the
better hand.
"""
import sys
import copy
...
class PokerHand(Hand):
    """Classifies a poker hand from a list of cards. The number of cards
    ...
    def __init__(self, *cards):
        """cards: Card tuple or list, a seq to init poker hand obj"""
        super().__init__()
        for card in cards: # if cards exist init poker hand obj
            ...
            if isinstance(card, Card): # add the card to the cards list
                # can't use derived class method as it uses an attribute
                # that has not yet been initialized by the ctor
                super().add_card(card)
            else:
        ...
        if cards: # if cards passed as params, hand needs classification
            self.classify()

    def add_card(self, card):
        """Override base class method.

        Add check to make sure num of cards and card type are correct.
    ...
    def classify(self, normal_flow = True, print_error = True):
        ...
        return: list (see def of self.__hand), the classified hand
        """
        ...
        if not normal_flow or self.__update(print_error):
            self.__reset(normal_flow) # reset data attributes

            # iterate over all cards and populate data structures for
            # suits and ranks
            ...
            self.__rank() # check for 4 of a kind, full house, straight,
                          # 3 of a kind, 2 pair or pair
    ...
    def compare(self, other):
        ...
        if other is self:
            ...
            desc = "\nresult\n======\n"
            # execute the comparison operators first to make sure there
            # exist valid hands
            result = self == other
            if result:
    ...
    def string(self, base = True):
        ...
        if self.classify(print_error = False): # if no error
            ...
            if base:
                desc += '\n'
            desc += f"\n{self.label:{PokerHand._LABEL_WIDTH}}: "\
                    f"{_description(self.__hand[1])}\n"
    ...
    def __repr__(self):
        """Called when calling repr(obj) on a poker hand object.
    ...
    def __call__(self, htype):
        """See doc of called method."""
        return self.has(htype)

    def __len__(self):
        """Called when calling len(obj) on a poker hand object.
    ...
    def __eq__(self, other):
        ...
        # Two hands are equal if the hand types are equal and the ranks
        # of the cards are equal. If rest of hand exists, card ranks
    ...
    def __lt__(self, other):
        ...
        return: bool or NotImplemented
                ...
                NotImplemented: if there's a parameter error or any hand
                                is incomplete
        """
        if not isinstance(other, PokerHand):
            print(f"error: 'other' = '{other}' must be of type "
        ...
        if self.__hand and other_hand:
            if self.__hand[0] < other_hand[0]:
                return True
            if self.__hand[0] == other_hand[0]: # same hand type

                # compare cards of hands
                cmp = _lt(self.__hand[1], other_hand[1])
                if cmp is None: # cards are equal
                    # compare rest of hand if it exists
                    return bool(_lt(self.__hand[2], other_hand[2])) \
                           if len(self.__hand) == PokerHand.__MAXLEN_HAND \
                           else False
                return cmp
        else:
    ...
    def __le__(self, other):
        ...
        return: bool or NotImplemented
                bool          : True if poker hand <= other poker hand
                NotImplemented: if there's a parameter error or any hand
                                is incomplete
        """
        return cmp_lt \
    ...
    def __iter__(self):
        """Called whenever iterator of poker hand object is requested.
    ...
    def __update(self, print_error):
        ...
        return: bool, True if hand needs classification and no error
        """
        ...
        # check if cards have been updated
        if sorted(self.__cards_copy) == sorted(self.cards):
            return False
    ...
    def __reset(self, normal_flow = False):
        ...
        """
         ...
         # if False, enable performance optimizations
        self.__normal_flow = normal_flow
        ...
        # Hand classified from list of cards. Structure is as follows:
        #
        # first element : int, the hand type
        # second element: list of Card, the hand itself
        # third element : list of Card if any, the rest of hand if any
        #
        # By poker rules, hand + rest of hand = 5 cards. Thus, if a hand
        # is 5 cards already, e.g. full house, there's no rest of hand.
        self.__hand = []

    def __suit(self):
        """Classify hand based on suit, i.e. straight flush and flush."""
        # If there are 7 cards, at least MIN_NUM_CARDS are required to
        # have a suit hand. That means that a maximum of 3 different
        # suits are possible out of 7 cards, provided a suit hand
        # exists. In general, if there are 'n' cards,
        # 'n - MIN_NUM_CARDS + 1' is the maximum number of suits
        # possible, provided a suit hand exists.
        if len(self.__suits) <= len(self.cards) - PokerHand.MIN_NUM_CARDS + 1:
            for hand in self.__suits.values(): # iterate over suits

                # suit hand exists
                if len(hand) >= PokerHand.MIN_NUM_CARDS:
                    # the hand has to be sorted descendingly in order to
                    # determine if it is a straight flush and have the
                    # highest cards first
                    hand.sort(reverse = True)

                    ace = hand[-1].rank == 1
                    # check hand is straight flush and ace first card
                    # ace, king, queen, jack, 10
                    if ace and hand[PokerHand.__SEQUENCE-2].rank == 10:
                        # insert ace as first card
                    ...
                    # Suppose the cards are 10,8,6,5,4,3,2. The straight
                    # flush is 6,5,4,3,2. Then, the algorithm steps are:
                    #                             is 10 == 4 + 4 -> no
                    #                             is  8 == 3 + 4 -> no
                    #                             is  6 == 2 + 4 -> yes
                    # So, out of 7 cards there's a max of 3 iterations.
                    # In general, if there are 'n' cards, the max num of
                    # iterations is 'n - SEQUENCE + 1'.
                    ...
                    if ace: # it's a flush and last card is ace
                        # insert ace as 1st card
                        hand.insert(0, hand.pop())

                    # add flush
                    self.__add(PokerHand.FLUSH, hand[:PokerHand.MIN_NUM_CARDS])
                    return
    ...
    def __rank(self):
        ...
        if not self.__hand:
            ...
            # ranks[i] -> single rank is tuple: (int,
            #                                    list of Card or hand)
            #      [0] -> first element of tuple, i.e. rank of the hand
            if ranks[-1][0] == 1: # save the ace card if any
                # ranks[i]->single rank is tuple: (int,
                #                                  list of Card or hand)
                #      [1]->second element of tuple, i.e. the hand
            ...
            # There is no straight so if an ace card exists, make the
            # ace hand the first hand in the list of hands. This is
            # necessary, as for example a pair of aces is preferred over
            # a pair of kings in a full house.
            if ace:
    ...
    def __straight(self, ranks, ace):
        ...
        if len(ranks) >= PokerHand.__SEQUENCE:
            # check if hand is a straight an ace is first card

            # ace, king, queen, jack, 10
            if ace and ranks[PokerHand.__SEQUENCE-2][0] == 10:
                hand = []
            ...
            # Same exact logic as per straight flush. See comments for
            # adding a straight flush that does not have an ace.
            for i in range(len(ranks) - PokerHand.__SEQUENCE + 1):
                # ranks[i]->single rank is tuple: (int,
                #                                  list of Card or hand)
                #      [0]->first element of tuple, i.e. rank of hand
                high_card_rank = ranks[i+PokerHand.__SEQUENCE-1][0] + \
                                 PokerHand.__SEQUENCE - 1
                if ranks[i][0] == high_card_rank:
                    hand = []
                    # to get explanation for ranks[j][1][0] see comments
                    # above for adding a straight that contains an ace
    ...
    def __full_house(self, hand, same_hand, complementary_hand):
        ...
        # if complementary hand exists, add full house
        if complementary_hand:
            if len(hand) == 2: # pair
        ...
        # a three exists already in 'same_hand' and 'hand' is another
        # three so add a full house (the highest three is 'same_hand')
        if len(same_hand) == 3:
    ...
    def __rest(self):
        ...
        # only add the rest of the hand once!
        if len(self.__hand) == PokerHand.__MAXLEN_HAND - 1:
            # If hand has length < MIN_NUM_CARDS add to it rest of cards
            # in order of rank, so that 'hand + rest = MIN_NUM_CARDS'.
            rest = PokerHand.MIN_NUM_CARDS - len(self.__hand[1])
            if rest:
                # delete cards that are part of the hand to facilitate
                # extracting the rest of the hand
                ...
                # finally, add the rest of the hand
                self.__hand.append(self.__cards_copy[:rest])

                # restore original cards
                self.__cards_copy.extend(copy.deepcopy(self.__hand[1]))
...
def _eq(cards, other_cards):
    """Iterate over cards from different hands and compare their rank.
    ...
    for card, other_card in zip(cards, other_cards):
        # if card ranks different then hands different
        if card.rank != other_card.rank:
            return False

    return True

def _lt(cards, other_cards):
    """Compare cards from two hands based on rank and '<' operator.
